use bevy::{ecs::system::Query, prelude::{Commands, Entity, Transform, With, Without}, prelude::*};
use rand::Rng;

use crate::{components::{acceleration::Acceleration, enemy::Enemy, player::{self, Player}, projectile::Projectile, velocity::Velocity}, utils::collision::{check_collision, check_collision_with_velocity, check_swept_aabb_collision}};
  ...
  for (enemy_transform, mut enemy_velocity, _) in enemies.iter_mut() {
    for (mut projectile_transform,mut projectile, mut velocity) in projectiles.iter_mut() {
      ...
      let (collision, entry_time, exit_time, normalx, normaly) = check_swept_aabb_collision(
        ...
        enemy_transform,
        &Velocity { x: velocity.x * delta * 2., y: velocity.y * delta * 2. },
        &enemy_velocity,
        delta
      );
      ...
      if collision {
        ...
        projectile_transform.translation.x = projectile_transform.translation.x + velocity.x * delta * entry_time;
        projectile_transform.translation.y = projectile_transform.translation.y + velocity.y * delta * entry_time;
...
pub fn player_collision(
  ...
  for (enemy_transform, enemy_velocity, _) in enemies.iter_mut() {
    ...
    let (collision, entry_time, exit_time, normalx, normaly) = check_swept_aabb_collision(
      &player.0,
      enemy_transform,
      &Velocity { x: player.2.x * delta, y: player.2.y * delta },
      &enemy_velocity,
      delta
    );
